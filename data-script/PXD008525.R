...
name <- "PXD008525_hs_Khib_sites_p300_ko"
...
GSEABase::toGmt(gs, con = paste(dir, "PXD008525_hs_Khib_sites_p300_ko.gmt", sep = ""))
...
name <- "PXD008525_hs_Kac_sites_p300_ko"
description <- "Kac sites in p300 knockout with KO/WT ratio <-1"
genes <- df[5] %>% distinct() %>% unlist() %>% unname() %>% strsplit(split = ";") %>% unlist %>% unique
gs <- GSEABase::GeneSet(setName = name, shortDescription = description, geneIds = genes)

GSEABase::toGmt(gs, con = paste(dir, "PXD008525_hs_Kac_sites_p300_ko.gmt", sep = ""))
